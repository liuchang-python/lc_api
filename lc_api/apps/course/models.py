...
from lc_api.utils.model import BaseModel
...
class Course(BaseModel):
    ...
    def active_list(self):
        # 找到当前课程所对应的活动列表
        # 当前课程所参与的活动是否在时间范围内  活动结束时间必须小于当前时间
    ...
    def expire_real_price(self,expire_id):
        ...
        expire_obj = CourseExpire.objects.filter(is_delete=False, is_show=True, pk=expire_id).first()
        # print(expire_obj.price)
        price = expire_obj.price
        self.price = price
    ...
    def discount_name(self):
        ...
        if len(active_list) > 0:
            # 当前时间参与了活动才返回优惠类型名称
            active = active_list[0]
            # 根据关系表找到当前课程参与的活动，再找到当前课程所对应的优惠类型
