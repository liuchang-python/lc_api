                    ...
                    <div class="sale-time" v-if="course.discount_name">
                        ...
                        <p class="expire">距离结束：仅剩 {{parseInt(course.active_time/(24*3600))}}天
                            {{parseInt(course.active_time/3600%24)}}小时 {{parseInt(course.active_time/60%60)}}分 <span
                                class="second">{{parseInt(course.active_time%60)}}</span> 秒</p>
                    </div>
                    <p class="course-price" v-if="course.real_price">
                        <span>活动价</span>
                        <span class="discount">¥{{ course.real_price }}</span>
                        <span class="original">¥{{ course.price }}.00</span>
                    </p>
                    <p class="course-price" v-else>
                        <span class="discount">¥{{ course.price }}.00</span>
                    </p>
                    <div class="buy">
                        <div class="buy-btn">
                        ...
                        <div class="add-cart" @click="add_cart"><img src="/static/image/12.png" alt="">加入购物车</div>
                    </div>
                </div>
            </div>
            ...
            <div class="course-content">
                <div class="course-tab-list">
                    ...
                    <div class="tab-item" v-if="tabIndex===2">
                        ...
                        <div class="chapter-item" v-for="(chapter,index1) in chapter_list" :key="chapter.id">
                            <p class="chapter-title"><img src="/static/image/1.svg" alt="">第{{ index1 + 1 }}章
                                {{ chapter.name }}</p>
                            <ul class="lesson-list" v-for="(lesson,index2) in chapter.coursesections" :key="index2">
                                <li class="lesson-item">
                                    <p class="name"><span
                                        class="index">{{ index1 + 1 }}-{{ index2 + 1 }}</span>{{ lesson.name }}
                                        Vue基本介绍<span class="free" v-show="lesson.free_trail">免费</span>
                                    </p>
    ...
    methods: {
        ...
        add_cart() {
            ...
            this.$axios.post(this.$settings.HOST + "cart/option/", {
                ...
                headers: {
                    // 添加购物车需要认证，写到token才能请求到后台 jwt 后边有空格
        ...
        get_course_detail() {
            ...
            console.log(id);
            this.id = id;
            this.$axios.get(this.$settings.HOST + "course/detail/" + id).then(res => {
                console.log(res.data);
